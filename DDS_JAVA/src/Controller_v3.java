// Controller_v3.java
// 语义：S8 = “模型增量 Δ”（同一客户端多包先相加 → FedAvg → currentModel += Δ）
//      Q8/FP32 = “完整权重”（FedAvg → 覆盖 currentModel）
...
public class Controller_v3 {
    ...
    private static class ClientVec {
        ...
        final boolean isDelta; // true=S8(增量)；false=权重(Q8/FP32)
    ...
    private void applyAndPublish(List<ClientVec> cvs, int roundId) {
        ...
        if (anyWeights && !allDelta) {
            ...
            currentModel = result; modelDim = dim;
        } else if (allDelta) {
            if (currentModel == null) {
                System.err.println("[Controller_v3] WARNING: currentModel is null; bootstrap a weights round first!");
                int dim = cvs.get(0).v.length; currentModel = new float[dim]; modelDim = dim;
            }
        ...
        if (rc != ReturnCode_t.RETCODE_OK) System.err.println("[Controller_v3] write ModelBlob failed: " + rc);
        else System.out.println("[Controller_v3] published FP32 model, bytes=" + modelBytes.length);
    }

    // ===== 识别/解码 =====
    private static boolean isS8Sparse(byte[] d){ return d!=null && d.length>=4 && d[0]=='S'&&d[1]=='8'&&d[2]==0&&(d[3]&0xFF)==1; }
    private static boolean isQ8Dense (byte[] d){ return d!=null && d.length>=4 && d[0]=='Q'&&d[1]=='8'&&d[2]==0&&(d[3]&0xFF)==1; }

    private static float[] decodeS8SparseToFloat(byte[] blob) {
        ByteBuffer bb = ByteBuffer.wrap(blob).order(ByteOrder.LITTLE_ENDIAN);
        if (!(bb.get()=='S' && bb.get()=='8' && bb.get()==0 && (bb.get()&0xFF)==1)) throw new IllegalArgumentException("bad S8");
        int dim = bb.getInt(); int k = bb.getInt(); float sc = bb.getFloat();
        float[] out = new float[dim]; int[] idx = new int[k];
        for (int i=0;i<k;i++) idx[i]=bb.getInt();
        for (int i=0;i<k;i++) { int id=idx[i]; if (0<=id && id<dim) out[id]+= bb.get()*sc; }
        return out;
    }
    private static float[] decodeQ8ToFloat(byte[] blob) {
        ByteBuffer bb = ByteBuffer.wrap(blob).order(ByteOrder.LITTLE_ENDIAN);
        if (!(bb.get()=='Q' && bb.get()=='8' && bb.get()==0 && (bb.get()&0xFF)==1)) throw new IllegalArgumentException("bad Q8");
        int chunk = bb.getInt(); long totalL = bb.getLong(); int nChunks = bb.getInt();
        if (totalL>Integer.MAX_VALUE) throw new IllegalArgumentException("too big");
        int total = (int) totalL;
        float[] scales = new float[nChunks]; for(int i=0;i<nChunks;i++) scales[i]=bb.getFloat();
        float[] out = new float[total];
        for (int ci=0; ci<nChunks; ci++) {
            int s=ci*chunk, e=Math.min(s+chunk,total); float sc=scales[ci];
            for (int j=s; j<e; j++) out[j] = bb.get()*sc;
        }
        return out;
    }
