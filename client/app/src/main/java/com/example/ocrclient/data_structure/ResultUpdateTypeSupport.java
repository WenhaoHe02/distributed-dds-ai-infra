...
public class ResultUpdateTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < itemsTmpLen; ++i){
            ResultItemTypeSupport.get_instance().print_sample(sample.items.get_at(i));
        }
        return 0;
    }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (itemsLen != 0){
            for (int i = 0; i < itemsLen; ++i){
                ResultItem curEle = sample.items.get_at(i);
                offset += ResultItemTypeSupport.get_instance().get_sizeI(curEle, cdr, offset);
            }
        }
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.items.length(); ++i){
            if (ResultItemTypeSupport.get_instance().serializeI(sample.items.get_at(i),cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        ResultItem tmpitems = new ResultItem();
        for (int i = 0; i < sample.items.length(); ++i){
            if (ResultItemTypeSupport.get_instance().deserializeI(tmpitems, cdr) < 0){
    ...
    public TypeCode get_typecode(){
        ...
        s_typeCode = factory.create_struct_TC("ResultUpdate");
        if (s_typeCode == null){
