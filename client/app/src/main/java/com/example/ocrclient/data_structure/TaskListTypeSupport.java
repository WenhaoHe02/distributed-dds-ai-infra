...
public class TaskListTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < tasksTmpLen; ++i){
            TaskTypeSupport.get_instance().print_sample(sample.tasks.get_at(i));
        }
        return 0;
    }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (tasksLen != 0){
            for (int i = 0; i < tasksLen; ++i){
                Task curEle = sample.tasks.get_at(i);
                offset += TaskTypeSupport.get_instance().get_sizeI(curEle, cdr, offset);
            }
        }
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.tasks.length(); ++i){
            if (TaskTypeSupport.get_instance().serializeI(sample.tasks.get_at(i),cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        Task tmptasks = new Task();
        for (int i = 0; i < sample.tasks.length(); ++i){
            if (TaskTypeSupport.get_instance().deserializeI(tmptasks, cdr) < 0){
    ...
    public TypeCode get_typecode(){
        ...
        memberTc = (TypeCodeImpl)TaskTypeSupport.get_instance().get_typecode();
        if (memberTc != null)
        {
