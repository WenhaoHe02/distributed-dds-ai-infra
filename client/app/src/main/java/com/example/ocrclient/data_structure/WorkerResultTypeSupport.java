...
public class WorkerResultTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < resultsTmpLen; ++i){
            WorkerTaskResultTypeSupport.get_instance().print_sample(sample.results.get_at(i));
        }
        return 0;
    }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (resultsLen != 0){
            for (int i = 0; i < resultsLen; ++i){
                WorkerTaskResult curEle = sample.results.get_at(i);
                offset += WorkerTaskResultTypeSupport.get_instance().get_sizeI(curEle, cdr, offset);
            }
        }
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.results.length(); ++i){
            if (WorkerTaskResultTypeSupport.get_instance().serializeI(sample.results.get_at(i),cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        WorkerTaskResult tmpresults = new WorkerTaskResult();
        for (int i = 0; i < sample.results.length(); ++i){
            if (WorkerTaskResultTypeSupport.get_instance().deserializeI(tmpresults, cdr) < 0){
