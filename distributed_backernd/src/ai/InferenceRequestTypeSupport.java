...
public class InferenceRequestTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < input_blobTmpLen; ++i){
            ai.SingleTaskTypeSupport.get_instance().print_sample(sample.input_blob.get_at(i));
        }
        System.out.println("sample.timeout_ms:" + sample.timeout_ms);
        ai.KVListTypeSupport.get_instance().print_sample(sample.info);
        return 0;
    }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (input_blobLen != 0){
            for (int i = 0; i < input_blobLen; ++i){
                ai.SingleTask curEle = sample.input_blob.get_at(i);
                offset += ai.SingleTaskTypeSupport.get_instance().get_sizeI(curEle, cdr, offset);
            }
        }

        offset += CDRSerializer.get_untype_size(4, offset);

        offset += ai.KVListTypeSupport.get_instance().get_sizeI(sample.info, cdr, offset);
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.input_blob.length(); ++i){
            if (ai.SingleTaskTypeSupport.get_instance().serializeI(sample.input_blob.get_at(i),cdr) < 0){
        ...
        if (ai.KVListTypeSupport.get_instance().serializeI(sample.info,cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        ai.SingleTask tmpinput_blob = new ai.SingleTask();
        for (int i = 0; i < sample.input_blob.length(); ++i){
            if (ai.SingleTaskTypeSupport.get_instance().deserializeI(tmpinput_blob, cdr) < 0){
        ...
        if (ai.KVListTypeSupport.get_instance().deserializeI(sample.info, cdr) < 0){
    ...
    public TypeCode get_typecode(){
        ...
        memberTc = (TypeCodeImpl)ai.KVListTypeSupport.get_instance().get_typecode();
        if (memberTc == null){
