...
public class TaskListTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < tasksTmpLen; ++i){
            ai.SingleTaskTypeSupport.get_instance().print_sample(sample.tasks.get_at(i));
        }
        ...
        ai.KVListTypeSupport.get_instance().print_sample(sample.meta);
        return 0;
    }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (tasksLen != 0){
            for (int i = 0; i < tasksLen; ++i){
                ai.SingleTask curEle = sample.tasks.get_at(i);
        ...
        offset += ai.KVListTypeSupport.get_instance().get_sizeI(sample.meta, cdr, offset);
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.tasks.length(); ++i){
            if (ai.SingleTaskTypeSupport.get_instance().serializeI(sample.tasks.get_at(i),cdr) < 0){
        ...
        if (ai.KVListTypeSupport.get_instance().serializeI(sample.meta,cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        ai.SingleTask tmptasks = new ai.SingleTask();
        for (int i = 0; i < sample.tasks.length(); ++i){
            if (ai.SingleTaskTypeSupport.get_instance().deserializeI(tmptasks, cdr) < 0){
        ...
        if (ai.KVListTypeSupport.get_instance().deserializeI(sample.meta, cdr) < 0){
