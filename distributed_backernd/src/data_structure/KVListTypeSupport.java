...
public class KVListTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < valueTmpLen; ++i){
            data_structure.KVTypeSupport.get_instance().print_sample(sample.value.get_at(i));
        }
        return 0;
    }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (valueLen != 0){
            for (int i = 0; i < valueLen; ++i){
                data_structure.KV curEle = sample.value.get_at(i);
                offset += data_structure.KVTypeSupport.get_instance().get_sizeI(curEle, cdr, offset);
            }
        }
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.value.length(); ++i){
            if (data_structure.KVTypeSupport.get_instance().serializeI(sample.value.get_at(i),cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        data_structure.KV tmpvalue = new data_structure.KV();
        for (int i = 0; i < sample.value.length(); ++i){
            if (data_structure.KVTypeSupport.get_instance().deserializeI(tmpvalue, cdr) < 0){
    ...
    public TypeCode get_typecode(){
        ...
        memberTc = (TypeCodeImpl) data_structure.KVTypeSupport.get_instance().get_typecode();
        if (memberTc != null)
        {
