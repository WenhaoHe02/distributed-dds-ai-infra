...
public class WorkerResultTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        for (int i = 0; i < resultsTmpLen; ++i){
            data_structure.SingleResultTypeSupport.get_instance().print_sample(sample.results.get_at(i));
        }
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        if (resultsLen != 0){
            for (int i = 0; i < resultsLen; ++i){
                data_structure.SingleResult curEle = sample.results.get_at(i);
                offset += data_structure.SingleResultTypeSupport.get_instance().get_sizeI(curEle, cdr, offset);
            }
        }
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        for (int i = 0; i < sample.results.length(); ++i){
            if (data_structure.SingleResultTypeSupport.get_instance().serializeI(sample.results.get_at(i),cdr) < 0){
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        data_structure.SingleResult tmpresults = new data_structure.SingleResult();
        for (int i = 0; i < sample.results.length(); ++i){
            if (data_structure.SingleResultTypeSupport.get_instance().deserializeI(tmpresults, cdr) < 0){
    ...
    public TypeCode get_typecode(){
        ...
        s_typeCode = factory.create_struct_TC("ai.WorkerResult");
        if (s_typeCode == null){
        ...
        memberTc = (TypeCodeImpl) data_structure.SingleResultTypeSupport.get_instance().get_typecode();
        if (memberTc != null)
        {
