...
TOPIC_TRAIN_CMD     = "train/train_cmd"
TOPIC_CLIENT_UPDATE = "train/client_update"
TOPIC_MODEL_BLOB    = "train/model_blob"
...
class ControllerV3:
    ...
    def __init__(self, cfg: "ControllerV3.Config"):
        self.cfg = cfg
    ...
    def main(argv: List[str]):
        ...
        cfg = ControllerV3._load_config(argv[0])
        ctrl = ControllerV3(cfg)
    ...
    def _on_update(self, cu):
        ...
        with self._lock:
            ...
            cs = m.setdefault(cid, ControllerV3.ClientStream())
    ...
    def _collect_client_vectors(self, streams: Dict[int, "ClientStream"]) -> List["ClientVec"]:
        out: List[ControllerV3.ClientVec] = []
        ...
        for cid, cs in streams.items():
            ...
            out.append(ControllerV3.ClientVec(sum_vec, n, is_delta))
    ...
    def _decode_s4_to_float(blob: bytes) -> List[float]:
        ...
        import struct
        if not ControllerV3._is_s4_sparse(blob):
