...
class Client_v4:
    ...
    def load_config(conf_path: str):
        ...
        Client_v4.DOMAIN_ID = j.get("domain_id")
        Client_v4.CLIENT_ID = j.get("client_id")
        Client_v4.NUM_CLIENTS = j.get("num_clients")
        Client_v4.BATCH_SIZE = j.get("batch_size", 32)
        Client_v4.DATA_DIR = j.get("data_dir")
        Client_v4.PYTHON_EXE = j.get("python_exe", os.environ.get("PYTHON_EXE", "python"))
        Client_v4.TRAINER_PY = j.get("trainer_script")

        Client_v4.SPARSE_RATIO = j.get("sparse_ratio", 0.1)
        Client_v4.COMM_EVERY = j.get("comm_every", 0)
        Client_v4.SPARSE_FIRST_ROUND = j.get("sparse_first_round", False)

        Client_v4.INIT_MODEL_PATH = j.get("init_model_path", "").strip()
        Client_v4.WAIT_MODEL_MS = j.get("wait_model_ms", 8000)

        print(
            f"[Client_v3] cfg: domain={Client_v4.DOMAIN_ID} "
            f"client={Client_v4.CLIENT_ID} "
            f"num_clients={Client_v4.NUM_CLIENTS} "
            f"sparse_ratio={Client_v4.SPARSE_RATIO} "
            f"comm_every={Client_v4.COMM_EVERY} "
            f"wait_model_ms={Client_v4.WAIT_MODEL_MS} "
