...
def _wait_writer_matched(writer, min_matches, timeout_ms, tag="[barrier]"):
    ...
    while (time.time() * 1000.0 - start) < timeout_ms:
        try:
            ...
            logging.info(f"{tag} get_publication_matched_status error: {e}")
            break
        if st.current_count != last:
            logging.info(f"{tag} writer matched: current={st.current_count} "
        ...
        time.sleep(0.2)
    if not ok:
        logging.info(f"{tag} writer matched NOT ready: need>={min_matches}, got={last}")
    return ok
...
def _wait_reader_matched(reader, min_matches, timeout_ms, tag="[barrier]"):
    ...
    while (time.time() * 1000.0 - start) < timeout_ms:
        try:
            ...
            logging.info(f"{tag} get_subscription_matched_status error: {e}")
            break
        if st.current_count != last:
            logging.info(f"{tag} reader matched: current={st.current_count} "
                  f"total={st.total_count} change={st.current_count_change}")
            last = st.current_count
        if st.current_count >= min_matches:
            ok = True
        ...
        time.sleep(0.2)
    if not ok:
        logging.info(f"{tag} reader matched NOT ready: need>={min_matches}, got={last}")
    return ok
...
def ddp_barrier_verbose(zrdds_allgather,
    ...
    logging.info("========== [barrier] allgather ==========")
    ...
    try:
        ...
        logging.info(f"[barrier] seen ranks: {seen}")
        missing = [i for i in range(world) if i not in seen]
        if missing:
            ok = False
            logging.info(f"[barrier] MISSING ranks: {missing}")
    except TimeoutError as e:
        ok = False
        logging.info(f"[barrier] TIMEOUT: {e}")

    logging.info(f"========== [barrier] result: {'OK' if ok else 'FAILED'} ==========")
    return ok