...
# ---------- Utils ----------
EPS = 1e-12
def make_shard_indices(n_total:int, num_clients:int, client_id:int, seed:int, mode:str="contig", limit:Optional[int]=None) -> List[int]:
    g=torch.Generator().manual_seed(seed); perm=torch.randperm(n_total, generator=g).tolist()
    client_id = client_id % max(1, num_clients)
    if mode=="contig":
        shard=math.ceil(n_total/num_clients); start=client_id*shard; idxs=perm[start:start+shard]
    elif mode=="stride":
        idxs=perm[client_id::num_clients]
    else: raise ValueError("partition mode")
    if limit and limit>0: idxs=idxs[:limit]
    return idxs
...
def load_init_model(path: str, model: nn.Module):
    if not path or not os.path.exists(path): print(f"[PY] init_model {path} not found, skip"); return
    with open(path,"rb") as f:
        magic4=f.read(4); f.seek(0)
        if magic4.startswith(b'Q8'):
            header=f.read(4+1+4+8+4); magic4,ver,chunk,total,nChunks=struct.unpack('<4sBiqi',header)
            scales=np.frombuffer(f.read(nChunks*4),dtype=np.float32); qvals=np.frombuffer(f.read(total),dtype=np.int8)
            vec=np.empty(total,dtype=np.float32)
            for i in range(nChunks):
                s=i*chunk; e=min(s+chunk,total); vec[s:e]=qvals[s:e].astype(np.float32)*scales[i]
        elif magic4.startswith(b'S8'):
            header=f.read(4+1+4+4+4); magic4,ver,dim,k,scale=struct.unpack('<4sBiif',header)
            idxs=np.frombuffer(f.read(k*4),dtype=np.int32); vals=np.frombuffer(f.read(k),dtype=np.int8).astype(np.float32)*scale
...
def q8_dense_pack(vec_f32: np.ndarray, chunk: int, out_path: str) -> dict:
    total=int(vec_f32.size); chunk=max(1,int(chunk)); n_chunks=(total+chunk-1)//chunk
    scales=np.empty(n_chunks,dtype=np.float32); qvals=np.empty(total,dtype=np.int8)
    for i in range(n_chunks):
        s=i*chunk; e=min(s+chunk,total); block=vec_f32[s:e]
        maxabs=float(np.max(np.abs(block))) if block.size>0 else 0.0
        sc=(maxabs/127.0) if maxabs>0 else 1e-8; scales[i]=sc
        qvals[s:e]=np.clip(np.round(block/sc),-128,127).astype(np.int8)
    with open(out_path,"wb") as f:
        f.write(b'Q8\x00'); f.write(struct.pack('<B',1)); f.write(struct.pack('<i',chunk))
        f.write(struct.pack('<q',total)); f.write(struct.pack('<i',n_chunks))
        f.write(scales.tobytes(order='C')); f.write(qvals.tobytes(order='C'))
    return {"codec":"int8/dense","chunk":chunk,"fp32_elems":total,
            "bytes_fp32": total*4, "bytes_packed": (4+1)+4+8+4 + n_chunks*4 + total}

def _topk_mask(x: np.ndarray, k: int):
    if k<=0 or k>=x.size:
        idx=np.arange(x.size,dtype=np.int32); mask=np.ones_like(x,dtype=bool); return idx,mask
    kth=np.argpartition(np.abs(x),-k)[-k:]; ord_idx=kth[np.argsort(-np.abs(x[kth]))]
    mask=np.zeros_like(x,dtype=bool); mask[ord_idx]=True; return ord_idx.astype(np.int32), mask
...
def s8_sparse_pack_parallel(vals_f32: np.ndarray, dim: int, idx: np.ndarray, out_path: str, n_threads:int=4) -> dict:
    ...
    def qblock(b): return np.clip(np.round(b/scale),-128,127).astype(np.int8)
    blocks=np.array_split(vals_f32,n_threads)
    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=n_threads) as ex: q_blocks=list(ex.map(qblock, blocks))
    q=np.concatenate(q_blocks)
    with open(out_path,"wb") as f:
        f.write(b'S8\x00'); f.write(struct.pack('<B',1))
        f.write(struct.pack('<i',dim)); f.write(struct.pack('<i',int(idx.size))); f.write(struct.pack('<f',float(scale)))
        f.write(idx.astype('<i4').tobytes(order='C')); f.write(q.tobytes(order='C'))
    return {"codec":"int8_sparse","dim":dim,"k":int(idx.size),"scale":float(scale),
            "bytes_packed": (4+1)+4+4+4 + idx.size*4 + idx.size, "bytes_fp32": dim*4}

# ---------- Δ 状态（按“轮”独立文件，避免跨轮污染） ----------
def delta_state_paths(state_dir: str, round_id: int):
    os.makedirs(state_dir, exist_ok=True)
    return (os.path.join(state_dir, f"delta_residual_r{round_id:04d}.npy"),
            os.path.join(state_dir, f"delta_sent_accum_r{round_id:04d}.npy"))

def delta_load_states(state_dir: str, round_id: int, dim: int):
    rp, sp = delta_state_paths(state_dir, round_id)
    r = np.zeros(dim, dtype=np.float32); s = np.zeros(dim, dtype=np.float32)
    if os.path.exists(rp):
        try:
            r0=np.load(rp);
            if r0.size==dim: r=r0.astype(np.float32,copy=False)
        except Exception: pass
    if os.path.exists(sp):
        try:
            s0=np.load(sp);
            if s0.size==dim: s=s0.astype(np.float32,copy=False)
        except Exception: pass
    return r, s

def delta_save_states(state_dir: str, round_id: int, residual: np.ndarray, sent_accum: np.ndarray):
    rp, sp = delta_state_paths(state_dir, round_id)
    np.save(rp, residual.astype(np.float32)); np.save(sp, sent_accum.astype(np.float32))
...
def train_one_client(args):
    ...
    # 第 1 轮：FP32 / Q8（权重）；第 2 轮起：S8（Δ）
    p_ref = flatten_params(model).cpu().numpy()
    dim   = int(p_ref.size)

    # 每轮独立 Δ 状态
    residual, sent_accum = delta_load_states(args.state_dir, args.round, dim)

    global_step=0; send_count=0; last_stats=None
    if args.compress=="int8_sparse" and args.comm_every>0:
        os.makedirs(args.out, exist_ok=True)  # 目录用于流式
    ...
    for _ in range(args.epochs):
        for x,y in loader:
            ...
            if args.compress=="int8_sparse" and args.comm_every>0 and (global_step%args.comm_every)==0:
                ...
                warm = (args.round + 1)/float(args.dgc_warmup_rounds) if (args.dgc_warmup_rounds and args.round<args.dgc_warmup_rounds) else 1.0
                ...
                delta_save_states(args.state_dir, args.round, residual, sent_accum)
                send_count += 1
    ...
    if args.compress=="int8_sparse":
        if args.comm_every==0:
            ...
            idx,_ = _topk_mask(delta_ef, k); vals=delta_ef[idx].astype(np.float32)
            ...
            return len(ds), {"codec":"int8_sparse_stream","packets":int(send_count),
    ...
    elif args.compress=="fp32":
        vec = parameters_to_vector(model.parameters()).detach().float().contiguous().cpu().numpy()
        os.makedirs(os.path.dirname(args.out), exist_ok=True)
        with open(args.out,"wb") as f: f.write(vec.astype("<f4").tobytes(order='C'))
    ...
    else:  # int8 dense weights（备用）
        vec = parameters_to_vector(model.parameters()).detach().float().contiguous().cpu().numpy()
        stats = q8_dense_pack(vec, chunk=args.chunk, out_path=args.out); stats["stream"]=False
        return len(ds), stats

def main():
    ap=argparse.ArgumentParser(description="MNIST + S8 Delta v3 (1st round FP32)")
    ...
    ap.add_argument("--comm_every", type=int, default=1)   # 0=回合末单包；>0=流式
    ap.add_argument("--init_model", type=str, default=None)
    args=ap.parse_args()

    num_samples, stats = train_one_client(args)
    meta={"client_id":args.client_id,"num_clients":args.num_clients,"num_samples":int(num_samples), **stats}
